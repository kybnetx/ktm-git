...
#
# 1.  SBKabProvider - name of this controller
# 2.  sbkp - table abbreviation for mapping functions
...
# 5.  sbkpext_grid - actions of REST interface
# 6.  sbkpext - path to access actions of REST interface
#


package katama::Controller::SBKabProvider;
...
sub index : Private {
	...
	$c->response->body('Matched katama::Controller::SBKabProvider');
}
...
### ALL (no args) ###
sub sbkpext_grid_all : Path('/sbkpext') : Args(0) : ActionClass('REST') {
	my ( $self, $c ) = @_;
#[[
	my $info = $c->request->content_type;
	$c->log->debug("###### DEBUGGE: sbkpext_grid_all [ $info ]");
#]]
}

# GET
sub sbkpext_grid_all_GET  {
	my ( $self, $c ) = @_;
	my ( $m, @rowobjs, $row, %data );

#[[
	my $info = $c->request->content_type;
	$c->log->debug("###### DEBUGGE: sbkpext_grid_all_GET [ $info ]");
#]]
...
#[[
	my $info = $c->request->content_type;
	$c->log->debug("###### DEBUGGE: sbkpext_grid_all_POST [ $info ]");
...
#]]
	...
	$rowscount = $m->count( 'iskabprov' => 1 );

	# pickup requested records
	@rowobjs = $m->search( { 'iskabprov' => 1 }, { 'rows' => $limit, 'page' => ($start/$limit + 1) } );
...
sub sbkpext_grid : Path('/sbkpext') : Args(1) : ActionClass('REST') {
	my ( $self, $c, $uidx ) = @_;
#[[
	my $info = $c->request->content_type;
	$c->log->debug("###### DEBUGGE: sbkpext_grid /UIDX $uidx/ - [ $info ]");
#]]
}

# GET
sub sbkpext_grid_GET  {
	my ( $self, $c, $uidx ) = @_;
	my ( $m, $row, %data );

#[[
	my $info = $c->request->content_type;
	$c->log->debug("###### DEBUGGE: sbkpext_grid_GET /UIDX $uidx/ - [ $info ]");
#]]
...
#	if ( !defined($row) ) {
	if ( !defined($row) || $row->iskabprov != 1 ) {
		return $self->status_not_found( $c,
			message => "GET_notfound" );
	}

	# map result row into JSON data array
	$data{'sbkp'} = [ get_sbkp($row) ];
...
# SINGLE POST
sub sbkpext_grid_POST  {
	my ( $self, $c, $uidx ) = @_;
	my ( $m, $row, $data, $rec );

#[[
	my $info = $c->request->content_type;
	$c->log->debug("###### DEBUGGE: sbkpext_grid_POST /UIDX $uidx/ - [ $info ]");
#]]
	...
	# UPDATE or CREATE for multiple rows ( /sbkpext/m )
	if ( $uidx eq 'm' ) {
		return sbkpext_grid_multi_post( $self, $c, $data );
	}
...
# DELETE
sub sbkpext_grid_DELETE  {
	my ( $self, $c, $uidx ) = @_;
	my ( $m, $row );

#[[
	my $info = $c->request->content_type;
	$c->log->debug("###### DEBUGGE: sbkpext_grid_DELETE /UIDX $uidx/ - [ $info ]");
#]]
...
# we don't use PUT
*sbkpext_grid_PUT = *sbkpext_grid_POST;

# MULTI POST
sub sbkpext_grid_multi_post {
	my ( $self, $c, $data ) = @_;
	my ( $m, @rowobjs, $row, @recobjs, $rec, $datarec, $uidx, $guid );

#[[
	$c->log->debug("###### DEBUGGE: sbkpext_grid_multi_post");
#]]
...
### SB Kabel Provider ComboBox Data ###
...
### ALL (no args) ###
sub sbkpext_grid_sel : Path('/sbkpext_sel') : Args(0) : ActionClass('REST') {
	my ( $self, $c ) = @_;
#[[
	my $info = $c->request->content_type;
	$c->log->debug("###### DEBUGGE: sbkpext_grid_sel [ $info ]");
#]]
}

# GET
sub sbkpext_grid_sel_GET  {
	my ( $self, $c ) = @_;
	my ( $m, @rowobjs, $row, %data );

#[[
	my $info = $c->request->content_type;
	$c->log->debug("###### DEBUGGE: sbkpext_grid_sel_GET [ $info ]");
#]]
...
#	@rowobjs = $m->all();
	...
	for $row ( @rowobjs ) {
		# don't take dummy record which has uidx 0
		# next if ($row->uidx == 0);
		push( @{ $data{'sbkpsel'} }, get_sbkpsel($row) );
	}
...
#[[
	my $info = $c->request->content_type;
	$c->log->debug("###### DEBUGGE: sbkpext_grid_sel_POST [ $info ]");
	$c->log->debug("###### /START: $start/ - /LIMIT: $limit/");
#]]
...
# END ### SB Kabel Provider ComboBox Data ###
